...
export const SYSTEM_DICTIONARIES: Record<string, Record<string, { label: string; metadata?: Record<string, unknown> }>> = {
  ...
  business_type: { contract_create: { label: "新增合同" }, contract_update: { label: "编辑合同" }, funds_create: { label: "新增收款" }, funds_delete: { label: "删除收款" }, course_create: { label: "新增排课" }, course_delete: { label: "删除排课" }, holiday_course_impact: { label: "停课处理" }, attendance: { label: "考勤签到" }, charge: { label: "扣费确认" }, charge_reverse: { label: "取消扣费" }, refund_create: { label: "新增退费" }, refund_delete: { label: "删除退费" }, contract_refund: { label: "合同退费" }, product_price: { label: "编辑产品" }, performance: { label: "业绩分配" }, performance_adjust: { label: "业绩调整" }, leave: { label: "请假" }, makeup: { label: "补课" } },
