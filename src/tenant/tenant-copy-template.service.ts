...
export async function previewCopyToTemplate(schemaName: string) {
  ...
  if (changes.length === 0) return { changes: [], targetSchema: "demo_school", message: "该租户无 DSL 改动，无需复制" };

  let warning: string | undefined;
  for (const change of changes) {
    const { rows } = await pool.query(
      `select id from admin.dsl_version where schema_scope = 'tenant' and schema_name = 'demo_school' and target_type = $1 and target_code = $2 and status = 'active'`,
      [change.dsl_type, change.dsl_code]
    );
    if (rows.length > 0) {
      warning = `demo_school 已存在 ${change.dsl_type}/${change.dsl_code} 的 active 版本，复制将创建 draft 版本`;
      break;
    }
  }

  return { changes, targetSchema: "demo_school", warning };
}
  ...
  return withClient(async (client) => {
    ...
    for (const change of changes) {
      ...
      const { rows: maxRows } = await client.query(
        `select coalesce(max(version_no), 0) + 1 as next_version from admin.dsl_version where schema_scope = 'tenant' and schema_name = 'demo_school' and target_type = $1 and target_code = $2`,
        [change.dsl_type, change.dsl_code]
      );
      ...
      await client.query(
        `insert into admin.dsl_version(id, schema_scope, schema_name, target_type, target_code, version_no, status, change_type, change_summary, diff_json, snapshot_json, created_by_user_id)
         values($1,'tenant','demo_school',$2,$3,$4,'draft','copy_from_tenant',$5,'{}',$6,$7)`,
    ...
    await client.query(
      ...
      [randomUUID(), input.schemaName, input.operatorId, JSON.stringify({ source: input.schemaName, target: "demo_school", copiedCount: draftVersionIds.length })]
    );
